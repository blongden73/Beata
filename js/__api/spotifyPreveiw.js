...
var parsedJSON = require('./__json/dataSpotify.json');
    ...
    spotifyApi.getArtistTopTracks(parsedJSON.artists[i].Id, 'GB')
        .then(function(data) {
			...
			for (var j = 0; j < parsedJSON.artists.length; j++) {
				...
				if(parsedJSON.artists[j].artist === data.body.tracks[i].artists[0].name){
					//set preview url
					parsedJSON.artists[j].previewURL = data.body.tracks[i].preview_url;

...
var parsedJSON = require('./__json/AllMusicData.json');
var file = 'js/__json/dataSpotifyAM.json';
...
for (var i = 0; i < parsedJSON.artists.length; i++) {
    ...
    spotifyApi.getArtistTopTracks(receivedParse.Artists[i], 'GB')
        .then(function(data) {
			...
			//console.log(data.body.tracks[].artists[0]);
			
			for (var j = 0; j < parsedJSON.artists.length; j++) {
			if (receivedParse.ArtistName[j] == data.body.tracks[i].artists[0].name) {
			//console.log(receivedParse.ArtistName[j], data.body.tracks[i].preview_url);	
            spotifyInfo.preveiwURL.push(data.body.tracks[i].preview_url);
            spotifyInfo.previewName.push(data.body.tracks[i].artists[0].name);
